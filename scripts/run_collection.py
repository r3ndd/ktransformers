...
Runs prompts from data/prompt_suite.json against one long-lived SGLang server,
captures outputs/traces/logs per prompt, and aggregates traces into
    ...
    try:
        if server.poll() is not None:
            raise RuntimeError(
                ...
                f"Log tail:\n{_read_log_tail(server_log_file)}"
            )

        sigquit_failed, marker = _log_has_sigquit_failure(server_log_file)
        if sigquit_failed:
            raise RuntimeError(
        ...
        print(
            f"  [err] server log tail for {prompt_id}:\n{_read_log_tail(server_log_file)}",
            flush=True,
        )
...
def main() -> int:
    ...
    server_log_file = OUTPUT_DIR / "collection_server.log"
    ...
    for r in results:
        if not r.get("success"):
            continue
        trace_path = Path(str(r.get("trace_file", "")))
        if not trace_path.exists():
            r["success"] = False
            r["error"] = f"Trace file missing after server shutdown: {trace_path}"
